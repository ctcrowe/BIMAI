...
class IndexModel(nn.Module):
    def __init__(self):
        ...
        self.size_head = nn.Linear(1, b_embd, dtype = torch.float)
        self.type_head = nn.Linear(1, b_embd, dtype = torch.float)
        self.first_blocks = nn.Sequential(*[timed.TimedBlock(n_embd, n_head=n_head, block_size=block_size) for _ in range(n_layer)])
        self.blocks = nn.Sequential(*[static.Block(n_embd + (2 * b_embd), n_head = n_head) for _ in range(n_layer)])
        self.ln_f = nn.LayerNorm(n_embd + (2 * b_embd))
        self.lm_head = nn.Linear(n_embd + (2 * b_embd), len(class_map.items()))
    ...
    def forward(self, device, A, B, C, targets = None):
        ...
        x = torch.cat([x, type_emb, size_emb], dim=-1)
        x = self.blocks(x)
        x = self.ln_f(x)
